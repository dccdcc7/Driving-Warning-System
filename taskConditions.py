...
from TrafficLaneDetector.ufldDetector.utils import OffsetType, CurvatureType, L_BSDCollisionType, R_BSDCollisionType
...
class TaskConditions(object):
	...
	def __init__(self):
		...
		self.l_normaltimes = 0
		self.r_normaltimes = 0
		self.l_warningtimes = 0
		self.r_warningtimes = 0

	def _calibration_curve(self, vehicle_curvature, frequency=3, curvae_thres=500):
		"""
	...
	def _calc_direction(self, curvature, curvae_dir, curvae_thres):   # 判断曲率半径方向，即车道弯曲方向
		"""
	...
	def UpdateBsdStatus(self, boxes, w, h) : 
		"""
		Judging the state of the avg distance.
		...
		center = []
		for box in boxes:
			center.append([0.5*(box[0]+box[2]), min(box[1], box[3])])
		L_min_point = max((x for x in center if (x[0]<w)), key=lambda x: x[0], default=None)
		R_min_point = min((x for x in center if (x[0]>2*w)), key=lambda x: x[0], default=None)

		# if L_min_point:
		# 	print(L_min_point)
		# 	if (L_min_point[1] + 1.5*h/w*L_min_point[0] < 0.9*h):
		# 		self.l_bsdcollision_msg = L_BSDCollisionType.L_WARNING
		# 	elif (L_min_point[1] + 0.75*h/w*L_min_point[0] < 0.75*h):
		# 		self.l_bsdcollision_msg = L_BSDCollisionType.L_PROMPT
		# 	else:
		# 		self.l_bsdcollision_msg = L_BSDCollisionType.L_NORMAL
		# if R_min_point:
		# 	print(R_min_point)
		# 	if (R_min_point[1] - 1.5*h/w*(R_min_point[0]-2*w) < -0.6*h):
		# 		self.r_bsdcollision_msg = R_BSDCollisionType.R_WARNING
		# 	elif (R_min_point[1] - 0.75*h/w*(R_min_point[0]-2*w) < 0):
		# 		self.r_bsdcollision_msg = R_BSDCollisionType.R_PROMPT
		# 	else:
		# 		self.r_bsdcollision_msg = R_BSDCollisionType.R_NORMAL
		# if L_min_point:
		# 	# print("L_min_points:", L_min_point)
		# 	if (L_min_point[1] + 0.5*h/w*L_min_point[0] < 0.7*h):
		# 		# print("AAAAAAAAAAAAAAAA:", L_min_point)
		# 		self.l_normaltimes = 0
		# 		self.l_warningtimes += 1
		# 		if self.l_warningtimes > 7:
		# 			self.l_bsdcollision_msg = L_BSDCollisionType.L_WARNING
		# 	else:
		# 		self.l_warningtimes = 0
		# 		self.l_normaltimes += 1
		# 		if self.l_normaltimes > 3:
		# 			self.l_bsdcollision_msg = L_BSDCollisionType.L_NORMAL
		# else:
		# 	self.l_warningtimes = 0
		# 	self.l_normaltimes += 1
		...
		# 		self.l_bsdcollision_msg = L_BSDCollisionType.L_NORMAL
		# if R_min_point:
		# 	if (R_min_point[1] - 0.5*h/w*(R_min_point[0]-2*w) < 0.2*h):
		# 		self.r_normaltimes = 0
		# 		self.r_warningtimes += 1
		# 		if self.r_warningtimes > 7:
		# 			self.r_bsdcollision_msg = R_BSDCollisionType.R_WARNING
		# 	else:
		# 		self.r_warningtimes = 0
		# 		self.r_normaltimes += 1
		# 		if self.r_normaltimes > 3:
		# 			self.r_bsdcollision_msg = R_BSDCollisionType.R_NORMAL
		# else:
		# 	self.r_warningtimes = 0
		# 	self.r_normaltimes += 1
		...
		if L_min_point:
			# print("L_min_points:", L_min_point)
			if 0.3*h < L_min_point[1] and 0.3*w < L_min_point[0]:
				# print("AAAAAAAAAAAAAAAA:", L_min_point)
				self.l_normaltimes = 0
				self.l_warningtimes += 1
				if self.l_warningtimes > 3:
					self.l_bsdcollision_msg = L_BSDCollisionType.L_WARNING
			else:
				self.l_warningtimes = 0
				self.l_normaltimes += 1
			...
			if self.l_normaltimes > 3:
				self.l_bsdcollision_msg = L_BSDCollisionType.L_NORMAL
		if R_min_point:
			if 0.3*h < R_min_point[1] and 2*w+0.7*w > R_min_point[0]>2*w:
				self.r_normaltimes = 0
				self.r_warningtimes += 1
				if self.r_warningtimes > 3:
					self.r_bsdcollision_msg = R_BSDCollisionType.R_WARNING
			else:
				self.r_warningtimes = 0
				self.r_normaltimes += 1
