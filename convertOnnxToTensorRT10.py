...
parser.add_argument('--input_onnx_model', '-i', default="./ObjectDetector/models/yolo11.onnx", type=str, help='Onnx model path.')
parser.add_argument('--output_trt_model', '-o', default="./ObjectDetector/models/yolo11.trt", type=str, help='Tensorrt model path.')
...
class EngineBuilder:
	...
	def __init__(self, verbose : bool = False, workspace : int = 1):
		...
		print(self.colorstr("馃憠 Starting export with TensorRT Version : "), trt.__version__)
		self.builder = trt.Builder(self.logger)
		self.config = self.builder.create_builder_config()
		self.config.set_memory_pool_limit(trt.MemoryPoolType.WORKSPACE, int(workspace * (1 << 30)))
	...
	def create_network(self, onnx_model_path : str):
		...
		print(self.colorstr('cyan', '馃憠 Loading the ONNX file...'))
		...
		print(self.colorstr('magenta', 'underline', '鉂勶笍  Network Description: 鉂勶笍'))
	...
	def create_engine(self, trt_model_path : str, calib_image_path: Optional[str] = None):
		...
		print(self.colorstr('馃憠 Building the TensorRT engine. This would take a while...'))
		serialized_engine = self.builder.build_serialized_network(self.network, self.config)
		if serialized_engine is not None:
			print(self.colorstr('馃憠 Completed creating engine.'))
		
		try:
			with open(trt_model_path, 'wb') as f:
				f.write(serialized_engine)
		except Exception as e:
			print(self.colorstr('red', f'Eexport failure 鉂?: {e}'))

		convert_time = time.time() - start
		print(self.colorstr(f'\nExport complete success 鉁?{convert_time:.1f}s'
